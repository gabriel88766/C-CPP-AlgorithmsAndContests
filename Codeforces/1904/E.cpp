#include <bits/stdc++.h>
typedef long long int ll;
typedef unsigned long long int ull;
const ll INF_LL = 0x3f3f3f3f3f3f3f3f, MOD = 998244353; //1e9+7
const int INF_INT = 0x3f3f3f3f;
const long double PI = acosl(-1.), EPS = 1e-9; 
using namespace std;

const int N = 2e5+3, M = 20; 
vector<int> adj[N];
int rt = 1, anc[N][M], h[N], ord[N], sub[N], inv[N];
int cnt = 0;

void dfs(int u, int par){ 
    ord[u] = ++cnt;
    inv[cnt] = u;
    sub[u] = 1;
    anc[u][0] = par;
    h[u] = h[par] + 1;
    for(auto x : adj[u]){
        if(x != par){
            dfs(x, u);
            sub[u] += sub[x];
        }
    } 
}

void build(int n){
    h[0] = -1;
    dfs(rt, 0);
    for(int j = 1; j < M; j++){
        for(int i = 1; i <= n; i ++){
            anc[i][j] = anc[anc[i][j-1]][j-1];
        }
    }
}

int lca(int a, int b){ 
    if(h[a] > h[b]) swap(a, b);
    int diff = h[b] - h[a];
    for(int i = M-1; i>=0;i--) if(diff & (1 << i)) b = anc[b][i];
    if(a == b) return a;
    for(int i= M-1;i >= 0;i--){
        while(anc[a][i] != anc[b][i]) a = anc[a][i], b = anc[b][i];
    }
    return anc[a][0];
}

int st[4*N], lz[4*N];
int n;

void build2(int l = 1, int r = n, int p = 1){
    if(l == r){ st[p] = h[inv[l]]; return; }
    build2(l, (l+r)/2, 2 * p);
    build2((l+r)/2 + 1, r, 2 * p + 1);
    st[p] = max(st[2 * p], st[2 * p + 1]); //some operation
}

void push(int l, int r, int p){
    if(lz[p]){ //0 can be assigned? change!
        st[p] += lz[p]; //RMQ = lz, RSQ, = (r-l+1)*lz
        if(l != r){
            lz[2 * p] += lz[p]; // += increment = update
            lz[2 * p + 1] += lz[p];
        } 
        lz[p] = 0;
    }
}

int query(int i, int j, int l = 1, int r = n, int p = 1){
    push(l, r, p);
    if(j < l || i > r) return 0; //identity element
    if(j >= r && i <= l) return st[p];
    return max(query(i, j, l, (l + r)/2, 2 * p), query(i, j, (l + r)/2 + 1, r, 2 * p + 1)); //some operation
}

void update(int i, int j, int value, int l = 1, int r = n, int p = 1){
    push(l, r, p);
    if(j < l || i > r) return;
    if(l >= i && r <= j) {lz[p] = value; push(l, r, p); return;}
    update(i, j, value, l, (l + r)/2, 2 * p);
    update(i, j, value, (l + r)/2 + 1, r, 2 * p + 1);
    st[p] = max(st[2 * p], st[2 * p + 1]); //some operation
}

vector<int> ans;
struct Query{
    vector<int> nds;
    int i;
};
vector<vector<Query>> vq;

void dfs_2(int u,int p){
    for(auto x : vq[u]){
        int cans = 0;
        int hpn = 0;
        for(auto no : x.nds){
            if(lca(no, u) == no && h[no] > h[hpn]) hpn = no;
        }
        int ht = h[u] - h[hpn] - 1;
        int aux = u;
        for(int i=0;i<M;i++) if((1 << i) & ht) aux = anc[aux][i];
        hpn = aux;
        vector<int> imp;
        for(auto no : x.nds){
            if(lca(no, u) != no){
                if(!hpn) imp.push_back(no);
                else if(ord[hpn] < ord[no] && ord[hpn] + sub[hpn] > ord[no]) imp.push_back(no);
            }
        }
        sort(imp.begin(), imp.end(), [&](int u, int v){ return ord[u] < ord[v];});
        int l = ord[hpn];
        for(auto no : imp){
            if(ord[no] > l) cans = max(cans, query(l, ord[no]-1));
            l = max(l, ord[no] + sub[no]);
        }
        if(l < ord[hpn] + sub[hpn]) cans = max(cans, query(l, ord[hpn] + sub[hpn] - 1));
        ans[x.i] = cans;
    }
    for(auto v : adj[u]){
        if(v != p){
            //modify seg tree
            update(1, ord[v]-1, 1);
            update(ord[v], ord[v]+sub[v]-1, -1);
            update(ord[v]+sub[v], n, 1);
            dfs_2(v, u);
            update(1, ord[v]-1, -1);
            update(ord[v], ord[v]+sub[v]-1, 1);
            update(ord[v]+sub[v], n, -1);
        }
    }
}
//cout << fixed << setprecision(6)
int main(){
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);
    //freopen("in", "r", stdin); //test input
    int q;
    cin >> n >> q;
    for(int i=1;i<n;i++){
        int a, b;
        cin >> a >> b;
        adj[a].push_back(b);
        adj[b].push_back(a);
    }
    build(n);
    build2();
    vq.resize(n+1);
    ans.resize(q);
    for(int i=0;i<q;i++){
        int x, k;
        cin >> x >> k;
        vector<int> vs(k);
        for(int j=0;j<k;j++){
            cin >> vs[j];
        }
        vq[x].push_back({vs, i});
    }
    dfs_2(1, 0);
    for(auto x : ans) cout << x << "\n";
}
