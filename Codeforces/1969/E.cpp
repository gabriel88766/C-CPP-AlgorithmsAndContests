...
struct SegmentTree { //1-based
    ...
    ll query_min(int i, int j, int l, int r, int p){
        if(j < l || i > r) return INF_LL; //identity element
        if(j >= r && i <= l) return T[p].min1;
        push(l, r, p);
        return min(query_min(i, j, l, (l + r)/2, 2 * p), query_min(i, j, (l + r)/2 + 1, r, 2 * p + 1)); //some operation
    }
    ll query_min(int i, int j){
        return query_min(i, j, 1, n, 1);
    }
};
...
int main(){
    ...
    while(t--){
        ...
        SegmentTree st(n+1);
        for(int i=1;i<=n;i++){
            auto it = lower_bound(vc[v[i]].begin(), vc[v[i]].end(), i);
            int lo, hi;
            if(it == vc[v[i]].begin()) lo = 1;
            else lo = *prev(it) + 1;
            if(next(it) == vc[v[i]].end()) hi = n;
            else hi = *next(it) - 1;
            st.update_chmax(lo, i, hi);
            if(st.query_min(1, i) < i){
                ans++;
                st.update_chmax(1, i, n);
            }
        }
        cout << ans << "\n";
    }
