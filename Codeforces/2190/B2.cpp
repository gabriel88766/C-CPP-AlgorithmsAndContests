...
int main(){
    ...
    while(t--){
        ...
        for(int i=1;i<=n;i++){
            for(int x=i;x>=0;x--){
                for(int sz=i;sz>=x;sz--){
                    for(int val=i;val>=0;val--){
                        if(x == sz){
                            if(val == sz && x != 0 && s[i] == '(') dp[x][x][x] += dp[x-1][x-1][x-1];
                        }else{
                            if(s[i] == '('){
                                if(x != sz - 1 && val != 0) dp[x][sz][val] += dp[x][sz-1][val-1];
                            }else{
                                dp[x][sz][val] += dp[x][sz-1][val+1];
                                if(val == 0 && 2*x + 2 < sz) ans += dp[x][sz-1][val+1] * (sz-2);
                            }
                        }
