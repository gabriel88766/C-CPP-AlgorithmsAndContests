            ...
            if(nums.size() && okp){
                if(nums.size() <= x){
                    ...
                    if(p == nums.size()){
                        ...
                        for(int i=0;i<p;i++){
                            aux.push_back(ansv.back());
                            ansv.pop_back();
                        }
                        p = 0;
                        for(int j=0;j<x;j++){
                            if(p < nums.size() && nums[p] == b[i][j]){
                                mrg.push_back(aux[p]);
                                p++;
                            }else{
                                mrg.push_back({b[i][j], i+1});
                            }
                        }
                        while(mrg.size()){
                            ansv.push_back(mrg.back());
                            mrg.pop_back();
                        }
                        nums.clear();
                ...
                ans += x;
                for(int j=m-x;j<=m-1;j++){
                    nums.push_back(a[i][j]);
                    ansv.push_back({b[i][j - (m-x)], i+1});
                }
