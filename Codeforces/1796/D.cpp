#include <bits/stdc++.h>
typedef long long int ll;
typedef unsigned long long int ull;
const ll INF_LL = 0x3f3f3f3f3f3f3f3f, MOD = 998244353; //1e9+7
const int INF_INT = 0x3f3f3f3f;
const long double PI = acosl(-1.), EPS = 1e-9; 
using namespace std;

const int N = 2e5+6;
ll st[4*N], lz[4*N], st2[4*N];
int n;

void clear(int l = 1, int r = n, int p = 1){
    st[p] = st2[p] = lz[p] = 0;
    if(l == r){  return; }
    clear(l, (l+r)/2, 2 * p);
    clear((l+r)/2 + 1, r, 2 * p + 1);
}

void push(int l, int r, int p){
    if(lz[p]){ //0 can be assigned? change!
        st[p] += lz[p]; //RMQ = lz, RSQ, = (r-l+1)*lz
        st2[p] += (ll)(r-l+1)*lz[p];    
        if(l != r){
            lz[2 * p] += lz[p]; // += increment = update
            lz[2 * p + 1] += lz[p];
        } 
        lz[p] = 0;
    }
}

ll query(int i, int j, int l = 1, int r = n, int p = 1){
    push(l, r, p);
    if(j < l || i > r) return INF_LL; //identity element
    if(j >= r && i <= l) return st[p];
    return min(query(i, j, l, (l + r)/2, 2 * p), query(i, j, (l + r)/2 + 1, r, 2 * p + 1)); //some operation
}

ll query2(int i, int j, int l = 1, int r = n, int p = 1){
    push(l, r, p);
    if(j < l || i > r) return 0; //identity element
    if(j >= r && i <= l) return st2[p];
    return query2(i, j, l, (l + r)/2, 2 * p) + query2(i, j, (l + r)/2 + 1, r, 2 * p + 1); //some operation
}

void update(int i, int j, ll value, int l = 1, int r = n, int p = 1){
    push(l, r, p);
    if(j < l || i > r) return;
    if(l >= i && r <= j) {lz[p] = value; push(l, r, p); return;}
    update(i, j, value, l, (l + r)/2, 2 * p);
    update(i, j, value, (l + r)/2 + 1, r, 2 * p + 1);
    st[p] = min(st[2 * p], st[2 * p + 1]); 
    st2[p] = st2[2 * p] + st2[2 * p + 1];
}


//cout << fixed << setprecision(6)
int main(){
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);
    //freopen("in", "r", stdin); //test input
    int t;
    cin >> t;
    while(t--){
        int k;
        ll x;
        cin >> n >> k >> x;
        clear();
        vector<ll> v(n+2);
        for(int i=1;i<=n;i++){
             cin >> v[i];
        }
        if(x >= 0){
            //better to take the max #
            ll ans = 0;
            for(int i=1;i<=n;i++){
                if(i > k) update(i-k, n, -2*x);
                update(i, n, x+v[i]);
                ans = max(ans, query(i, i) - min(query(1, i-1), 0LL));
            }
            cout << ans << "\n";
        }else{
            //better to take the min #
            k = n - k;
            ll ans = 0;
            for(int i=1;i<=n;i++){
                if(i > k) update(i-k, n, 2*x);
                update(i, n, v[i]-x);
                ans = max(ans, query(i, i) - min(query(1, i-1), 0LL));
            }
            cout << ans << "\n";
        }

    }
}
