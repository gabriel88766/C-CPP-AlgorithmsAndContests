...
const __int128 max_v = 1'000'000'000'000'000LL;
struct Node {
	__int128 sum;   // Sum tag
	__int128 max1;  // Max value
	__int128 max2;  // Second Max value
	__int128 maxc;  // Max value count
	__int128 min1;  // Min value
	__int128 min2;  // Second Min value
	__int128 minc;  // Min value count
	__int128 lazy;  // Lazy tag
};
...
struct SegmentTree { //1-based
    ...
    SegmentTree(int n){
        this->n = n;
        vector<__int128> val(n+1, 0);
        T.resize(4*(n+1));
        build(val, 1, n, 1);
    }
    SegmentTree(vector<__int128> &val){ //val.size() >= 1
        n = val.size() - 1;
        T.resize(4*(n+1));
        build(val, 1, n, 1);
    }
    ...
    void build(vector<__int128> &val, int l, int r, int p){
        T[p].lazy = 0;
        if(l == r){
            T[p].maxc = T[p].minc = 1;
    ...
    void push_min(int l, int r, int p, __int128 b){
    ...
    void push_max(int l, int r, int p, __int128 b){
    ...
    void push_add(int l, int r, int p, __int128 b){
        T[p].lazy += b;
        T[p].sum += (r - l + 1) * b;
        T[p].max1 += b;
        T[p].min1 += b;
    ...
    void update_add(int i, int j, __int128 b, int l, int r, int p){ //ai -> ai + b
    ...
    void update_add(int i, int j, __int128 b){
        update_add(i, j, b, 1, n, 1);
    }

    void update_chmin(int i, int j, __int128 b, int l, int r, int p){ //ai -> min(ai, b)
    ...
    void update_chmin(int i, int j, __int128 b){
        update_chmin(i, j, b, 1, n, 1);
    }

    void update_chmax(int i, int j, __int128 b, int l, int r, int p){ //ai -> max(ai, b)
    ...
    __int128 query_sum(int i, int j, int l, int r, int p){
    ...
    __int128 query_sum(int i, int j){
...
int main(){
    ...
    while(q--){
        ll l, r, k;
        cin >> l >> r >> k;
        __int128 cs = st.query_sum(l, r);
        st.update_add(l, r, -k);
        st.update_chmax(l, r, 0);
        __int128 ns = st.query_sum(l, r);
        ll ans = cs - ns;
        cout << ans << "\n";
    }
}
