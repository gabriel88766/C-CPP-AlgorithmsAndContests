...
struct SegmentTree { //1-based
    ...
    void update_divide(int i, int j, ll b, int l, int r, int p){ //ai -> min(ai, b)
        if(j < l || i > r || T[p].max1 <= 0) return;
        if(i <= l && r <= j && (T[p].max2 < 0 || (T[p].max1/b != 0 && T[p].max2 == 0))){
            push_max(l, r, p, T[p].max1/b);
            return;
        }
        push(l, r, p);
        int m = (l + r)/2;
        update_divide(i, j, b, l, m, 2*p);
        update_divide(i, j, b, m+1, r, 2*p+1);
        merge(p);
    }
    void update_divide(int i, int j, ll b){
        update_divide(i, j, b, 1, n, 1);
    }
...
int main(){
    ...
    for(int i=1;i<=q;i++){
        ...
        if(tq == 1){
            cin >> x;
            s.update_divide(l, r, x);
        }else if(tq == 2){
            cin >> x;
            s.update_chmin(l, r, 0);
            s.update_chmax(l, r, x);
        }else{
            cout << s.query_sum(l, r) << "\n";
        }
        // s.printstate();
    }
}
