...
int main(){
    ...
    s = " " + s;
    for(int i=1;i<=n;i++){
        if(s[i] == '(') st.update(i, n, 1);
        else st.update(i, n, -1);
    }
    for(int i=1;i<=q;i++){
        int t, l, r;
        cin >> t >> l >> r;
        if(t == 1){
            if(s[l] == '(') st.update(l, n, -1), st.update(r, n, 1);
            else st.update(l, n, 1), st.update(r, n, -1);
            if(s[r] == '(') st.update(r, n, -1), st.update(l, n, 1);
            else st.update(r, n, 1), st.update(l, n, -1);
            swap(s[l], s[r]);
        }else{
            int vl = st.query(l-1, l-1);
            int vr = st.query(r, r);
            int mn = st.query(l, r);
            if(mn == vl && vl == vr) cout << "Yes\n";
            else cout << "No\n";
        }
    }
}
