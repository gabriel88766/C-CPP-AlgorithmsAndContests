...
int main(){
    ...
    for(int i=1;i<=n;i++){
        int r, c, l;
        cin >> r >> c >> l;
        vt.push_back({r, c, l, i});
    }
    sort(vt.begin(), vt.end(), greater<tuple<int,int,int,int>>());
    SegmentTreeLazy<int, op, el> st(w+1);
    for(auto [r, c, l, j] : vt){
        int mc = c + l - 1;
        ans[j] = h - st.query(c, mc);
        st.update(c, mc, st.query(c, mc)+1);
    }
