...
int main(){
    ...
    int n, a, b, c;
    cin >> n >> a >> b >> c;
    vector<Mint> va(n+1), vb(n+1), vc(n+1);
    for(int i=0;i<=n;i+=a) va[i] = 1;
    for(int i=0;i<=n;i+=b) vb[i] = 1;
    for(int i=0;i<=n;i+=c) vc[i] = 1;
    auto xx = multiply2(va, vb);
    xx.resize(n+1);
    xx = multiply2(xx, vc);
    cout << xx[n] << "\n";


    for(int i=0;i<=n;i+=a) va[i] = invfat[i];
    for(int i=0;i<=n;i+=b) vb[i] = invfat[i];
    for(int i=0;i<=n;i+=c) vc[i] = invfat[i];
    xx = multiply2(va, vb);
    xx.resize(n+1);
    xx = multiply2(xx, vc);
    cout << xx[n] * fat[n] << "\n";
