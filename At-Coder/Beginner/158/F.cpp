...
int main(){
    ...
    vp[0] = {-INF_INT, -INF_INT};
    for(int i=1;i<=n;i++){
        cin >> vp[i].first >> vp[i].second;
    }
    sort(vp.begin(), vp.end());
    vp.push_back({2'000'000'000, 2'000'000'000});
    SegmentTree<int, op, el> nxt(n+1);
    for(int i=n;i>=1;i--){
        auto [x, d] = vp[i];
        int ind = lower_bound(vp.begin(), vp.end(), make_pair(x+d, -1)) - vp.begin();
        //find maximum in range [i, ind-1];
        nxt.update(i, ind);
        nxt.update(i, nxt.query(i, ind-1));
    }


    for(int i=1;i<=n;i++){
        cur += ss[i];
        ss[nxt.query(i, i)] += cur;
    }
    cur += ss[n+1];
    cout << cur << "\n";
}
